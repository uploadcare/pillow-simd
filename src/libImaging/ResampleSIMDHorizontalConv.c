    ...
    int xsize, int *xbounds, INT16 *kk, int kmax, int coefs_precision)
{
...
#if defined(__AVX2__)
    {
        ...
        for (; x < xmax; x ++) {
            ...
            pix = _mm256_inserti128_si256(_mm256_castsi128_si256(
                _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn0[x + xmin]))),
                _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn1[x + xmin])), 1);
            sss0 = _mm256_add_epi32(sss0, _mm256_madd_epi16(pix, mmk));

            pix = _mm256_inserti128_si256(_mm256_castsi128_si256(
                _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn2[x + xmin]))),
                _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn3[x + xmin])), 1);
            sss1 = _mm256_add_epi32(sss1, _mm256_madd_epi16(pix, mmk));
        }
...
#else
    {
        ...
        for (; x < xmax; x ++) {
            ...
            pix = _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn0[x + xmin]));
            sss0 = _mm_add_epi32(sss0, _mm_madd_epi16(pix, mmk));

            pix = _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn1[x + xmin]));
            sss1 = _mm_add_epi32(sss1, _mm_madd_epi16(pix, mmk));

            pix = _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn2[x + xmin]));
            sss2 = _mm_add_epi32(sss2, _mm_madd_epi16(pix, mmk));

            pix = _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn3[x + xmin]));
            sss3 = _mm_add_epi32(sss3, _mm_madd_epi16(pix, mmk));
        }
    ...
    int xsize, int *xbounds, INT16 *kk, int kmax, int coefs_precision)
{
    ...
    if (xmax < 8) {
        ...
        for (; x < xmax; x ++) {
            __m128i pix = _mm_cvtepu8_epi32(_mm_cvtsi32_si128(lineIn[x + xmin]));
